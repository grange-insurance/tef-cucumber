...
describe 'Runner, Unit' do
  ...
  describe 'instance level' do
    before(:each) do
      ...
      @default_cucumber_options = {formatters: {'Cucumber::Formatter::JsonExpanded' => ''}, options: ['--expand']}
      @baseline_task_data = {command: 'cucumber', cucumber_options: @default_cucumber_options}
      @test_task = {task_data: {}}
    end
    ...
    describe 'new behavior' do
      ...
      it 'executes a cucumber command by default' do
        ...
        expect(@executor).to have_received(:execute).with(@baseline_task_data.merge(command: 'cucumber')).once
      end
      ...
      it 'executes a different command, if provided' do
        ...
        expect(@executor).to have_received(:execute).with(@baseline_task_data.merge(command: explicit_command)).once
      end
      ...
      it 'can execute cucumber with bundler' do
        ...
        expect(@executor).to have_received(:execute).with(@baseline_task_data.merge(gemfile: gemfile_to_use, env_vars: {'BUNDLE_GEMFILE' => gemfile_to_use}, command: 'bundle exec cucumber')).once
      end
      ...
      it 'will add to the existing environmental variables, if provided, when using bundler' do
        ...
        expect(@executor).to have_received(:execute).with(@baseline_task_data.merge(gemfile: gemfile_to_use, env_vars: {'FOO' => 'BAR', 'BUNDLE_GEMFILE' => gemfile_to_use}, command: 'bundle exec cucumber')).once
      end
      ...
      it 'executes cucumber without bundler' do
        ...
        expect(@executor).to have_received(:execute).with(@baseline_task_data).once
      end
      ...
      it "treats the provided gemfile as relative to the task's working directory (if one is provided)" do
        ...
        expect(@executor).to have_received(:execute).with(@baseline_task_data.merge(working_directory: working_directory, gemfile: gemfile_to_use, env_vars: {'BUNDLE_GEMFILE' => "#{working_directory}/#{gemfile_to_use}"}, command: 'bundle exec cucumber')).once
