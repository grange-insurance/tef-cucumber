Then(/^results for the executed tests are stored by the keeper$/) do
  ...
  wait(180).for { get_tests_for_suite(@test_suite_guid).count }.to eq(expected_test_cases.count)

  stored_test_cases = get_tests_for_suite(@test_suite_guid)
  stored_test_cases.collect! { |test_case| test_case.filename.gsub('\\', '/') }
